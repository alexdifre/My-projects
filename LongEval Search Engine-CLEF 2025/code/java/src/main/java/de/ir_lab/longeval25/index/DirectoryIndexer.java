...
import de.ir_lab.longeval25.parse.DocumentParser;
...
public class DirectoryIndexer {
    ...
    public void index() throws IOException {
        ...
        // used to don't save duplicate docs
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                if (file.getFileName().toString().endsWith(extension)) {
                    ...
                    for (ParsedDocument pd : dp) {
                        ...
                        String docId = pd.getIdentifier();
                        if (processedDocIds.contains(docId))
                            continue;

                            processedDocIds.add(docId);  // add the id to the set
                            ...
                            doc.add(new StartField(pd.getStart()));
                            
                            // add the HIGHLIGHTS of the document
                            doc.add(new HighlightedField(pd.getHighlights()));
                            
                            // add TERMSVECTORS of the document
                            doc.add(new Field("content", pd.getBody(), fieldType));
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String docsPath = config.getString("collectionPath");
        final String indexPath = config.getString("indexPath");

        final String extension = "trec";
        ...
        DirectoryIndexer i = new DirectoryIndexer(a, new BM25Similarity(), ramBuffer, indexPath, docsPath, extension,
                                                  charsetName, expectedDocs, TrecParser.class);
